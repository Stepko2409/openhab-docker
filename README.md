...
[![Docker Version](https://img.shields.io/badge/version-4.1.1-blue)](https://hub.docker.com/repository/docker/openhab/openhab/tags?name=4.1.1)
...
* **Stable:** Thoroughly tested semi-annual official releases of openHAB. Use the stable version for your production environment if you do not need the latest enhancements and prefer a robust system.
  * `4.1.1` ([Release notes](https://github.com/openhab/openhab-distro/releases/tag/4.1.1))
...
If you are unsure about what your needs are, you probably want to use `openhab/openhab:4.1.1`.
...
`docker run --name openhab --net=host openhab/openhab:4.1.1`
...
`docker run --detach --name openhab openhab/openhab:4.1.1`
...
`docker run -it openhab/openhab:4.1.1`

#### Debug mode

The debug mode is started with the command:

`docker run -it openhab/openhab:4.1.1 ./start_debug.sh`
...
```shell
$ docker build --build-arg JAVA_VERSION=17 --build-arg OPENHAB_VERSION=4.1.1 --tag openhab/openhab .
...
```shell
$ docker buildx build --build-arg JAVA_VERSION=17 --build-arg OPENHAB_VERSION=4.1.1 --platform linux/arm/v7 --tag openhab/openhab --load .
```
